...
def copyGameState(Board):
    NewBoard = gameboard.GameBoard(Board.board, Board.width, Board.height, Board.spawnPool, Board.moveHistory, Board.Goal, Board.h)
    return NewBoard
...
def spawnFrequencyMapping(SpawnPool):
    ...
    for i in uniqueValues:
        spawnFrequency[i] =  (spawnFrequency[i] / float(len(SpawnPool)))
